...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 1400) }}
      style={{ fontSize: 11, color: copied ? 'var(--accent)' : 'var(--faint)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontWeight: 500, flexShrink: 0 }}>
      {copied ? 'Copied' : 'Copy'}
    </button>
  )
}

function Block({ label, text, dark }: { label: string; text: string; dark?: boolean }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: dark ? 'rgba(255,255,255,0.3)' : 'var(--accent)', marginBottom: 8 }}>{label}</div>
      <div style={{ fontSize: 14, lineHeight: 1.7, background: dark ? 'rgba(255,255,255,0.05)' : 'var(--bg)', padding: '14px 16px', borderRadius: 2, border: `1px solid ${dark ? 'rgba(255,255,255,0.08)' : 'var(--border)'}`, display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
        <span style={{ color: dark ? 'rgba(255,255,255,0.7)' : 'var(--ink)' }}>{text}</span>
...
function SingleOutput({ data, productName }: { data: Output; productName?: string }) {
    ...
    <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 2, padding: 32 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <ExportButton output={data} productName={productName} />
          <div style={{ display: 'flex', gap: 1, background: 'var(--border)' }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 16px', fontSize: 11, fontWeight: tab === t ? 600 : 400, textTransform: 'uppercase', letterSpacing: '0.06em', background: tab === t ? 'var(--white)' : 'var(--bg2)', color: tab === t ? 'var(--ink)' : 'var(--muted)', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
                {t}
              </button>
            ))}
          </div>
      ...
      {tab === 'copy' && (
        <div>
          ...
          {data.why_it_works && (
            <div style={{ background: 'var(--dark)', padding: 20, borderRadius: 2, marginTop: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: 8 }}>Why it works</div>
...
export default function OutputPanel({ output, productName }: { output: Record<string, unknown> | Record<string, unknown>[]; productName?: string }) {
    ...
    return (
      <div style={{ marginTop: 32 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--accent)' }}>All 5 stages generated</div>
          <ExportButton output={stages} productName={productName} />
        </div>
        ...
        <SingleOutput data={stages[activeStage]} productName={productName} />
      </div>
    )
  }
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      <SingleOutput data={output as Output} productName={productName} />
    </div>
  )
}
